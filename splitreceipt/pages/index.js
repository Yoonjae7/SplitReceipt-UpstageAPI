...
function parseItems(data) {
  ...
  if (result.length === 0) {
    return {
      items: [
        {name:'Demo Burger',price:12},
        {name:'Demo Pasta',price:15},
        {name:'Demo Juice',price:6}
      ],
      charges
    };
  }
...
export default function Home() {
  ...
  const extraTotal =
    (charges.tax || 0) +
    (charges.service || 0) +
    (charges.others || []).reduce((a, c) => a + (c.amount || 0), 0);
  ...
  const calcResults = () => {
    ...
    const perTax = (charges.tax || 0) / n;
    const perSvc = (charges.service || 0) / n;
  ...
  const maxTotal = panel === 4 ? Math.max(...Object.values(totals)) : 0;
  ...
  const reset = () => {
    ...
    setFile(null);
    setPreview(null);
    setCharges({ tax: 0, service: 0, others: [] });
    setPersonInput('');
    setError('');
    setPanel(1);
  };
  ...
  return (
    <>
      ...
      <style>{`
        ...
        .status-dot{width:8px;height:8px;border-radius:50%;background:${apiStatus==='online'?'#4ade80':'#ff6b35'};display:inline-block;margin-right:8px;${apiStatus==='online'?'box-shadow:0 0 0 2px rgba(74,222,128,0.3);animation:pulse 2s infinite;':''}}
        @keyframes pulse{0%,100%{box-shadow:0 0 0 2px rgba(74,222,128,0.3);}50%{box-shadow:0 0 0 6px rgba(74,222,128,0);}}
        ...
        .btn{font-family:'Syne',sans-serif;font-weight:700;font-size:13px;padding:12px 24px;border-radius:8px;border:none;cursor:pointer;transition:all 0.15s;letter-spacing:0.3px;}
        ...
        .remove-btn{cursor:pointer;color:var(--muted);font-size:16px;line-height:1;background:none;border:none;}
        .remove-btn:hover{color:var(--accent2);}
        .add-row{display:flex;gap:8px;align-items:center;}
        .text-input{background:var(--surface);border:1px solid var(--border);color:var(--text);font-family:'DM Mono',monospace;font-size:13px;padding:10px 14px;border-radius:8px;outline:none;transition:border-color 0.2s;}
        ...
        .person-check{display:flex;align-items:center;gap:5px;background:var(--surface2);border:1px solid var(--border);border-radius:100px;padding:4px 10px;cursor:pointer;font-size:11px;transition:all 0.15s;user-select:none;}
        .person-check.checked{background:rgba(200,241,53,0.12);border-color:var(--accent);}
        ...
        .warn-box{background:rgba(255,107,53,0.1);border:1px solid rgba(255,107,53,0.3);color:var(--accent2);border-radius:8px;padding:10px 14px;font-size:12px;margin-bottom:16px;}
        .summary-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(180px,1fr));gap:12px;margin-bottom:40px;}
        .summary-card{background:var(--surface);border:1px solid var(--border);border-radius:var(--radius);padding:20px;transition:border-color 0.2s;}
        .summary-card:hover{border-color:var(--accent);}
        .summary-name{font-family:'Syne',sans-serif;font-weight:700;font-size:16px;margin-bottom:4px;}
        .summary-amount{font-size:28px;font-weight:500;color:var(--accent);margin-bottom:12px;letter-spacing:-1px;}
        .summary-items{font-size:11px;color:var(--muted);line-height:1.8;}
        ...
        .bar-fill{height:100%;border-radius:4px;display:flex;align-items:center;padding:0 10px;font-size:11px;font-weight:500;color:#0e0e0e;transition:width 0.8s cubic-bezier(0.16,1,0.3,1);white-space:nowrap;}
        ...
        .preview-img{max-width:200px;max-height:200px;border-radius:8px;border:1px solid var(--border);object-fit:cover;margin-top:16px;}
      ...
      <div className="container">
        ...
        {panel === 2 && (
          <div>
            ...
            <div className="row-actions">
              <button className="btn btn-primary" disabled={people.length < 2} onClick={() => setPanel(3)}>
                Assign Items →
              </button>
        ...
        {panel === 3 && (
          <div>
            ...
            {hasUnassigned && <div className="warn-box">⚠ Some items have no one assigned yet</div>}
                  ...
                  return (
                    <tr key={i}>
                      ...
                      <td>
                        <div className="person-checks">
                          {people.map((name, pi) => (
                            <label
                              key={name}
                              className={`person-check${assigned.includes(name) ? ' checked' : ''}`}
                              onClick={e => {
                                e.preventDefault();
                                toggleAssign(i, name);
                              }}
                      ...
                      <td>
                        ...
                        {assigned.length > 1 && (
                          <span className="split-badge">÷{assigned.length}</span>
                        )}
                      </td>
                    </tr>
                  );
            ...
            <div className="charges-info">
              {charges.tax > 0 &&
                `Tax (~${itemsSubtotal > 0 ? ((charges.tax / itemsSubtotal) * 100).toFixed(1) : 0}%): ${fc(charges.tax)}`}
              {charges.service > 0 && ` · Service: ${fc(charges.service)}`}
            </div>
            <div className="row-actions">
              <button className="btn btn-primary" disabled={hasUnassigned} onClick={goCalculate}>
                Calculate Split →
              </button>
        ...
        {panel === 4 && (
          <div>
            ...
            <div className="summary-grid">
              {people.map((name, i) => (
                <div key={name} className="summary-card" style={{borderColor:COLORS[i%COLORS.length]+'44'}}>
                  <div className="summary-name" style={{color:COLORS[i%COLORS.length]}}>{name}</div>
                  <div className="summary-amount">{fc(totals[name])}</div>
                  <div className="summary-items">
                    {(breakdown[name] || []).map((b, j) => (
                      <div key={j}>
                        {b.name}
                        {b.count > 1 ? ` ÷${b.count}` : ''} = {fc(b.share)}
                      </div>
                    ))}
                  </div>
                </div>
            ...
            <div className="chart-section">
              ...
              {people.map((name, i) => {
                const pct = maxTotal > 0 ? (totals[name] / maxTotal) * 100 : 0;
                return (
            ...
            <div className="chart-section">
              ...
              <div className="pie-container">
                ...
                <div className="pie-legend">
                  {people.map((name, i) => {
                    const total = Object.values(totals).reduce((a, b) => a + b, 0);
                    const pct = total > 0 ? ((totals[name] / total) * 100).toFixed(1) : 0;
                    return (
